...
from easyai.data_loader.gan.gan_sample import GanSample
from easyai.data_loader.gan.gan_dataset_process import GanDatasetProcess


class GanDataloader(TorchDataLoader):

    def __init__(self, train_path, resize_type, normalize_type,
                 mean=0, std=1, image_size=(768, 320), data_channel=3):
        super().__init__(data_channel)
        self.image_size = image_size
        self.gan_sample = GanSample(train_path)
        self.gan_sample.read_sample()
        self.dataset_process = GanDatasetProcess(resize_type, normalize_type,
                                                 mean, std,
...
def get_gan_train_dataloader(train_path, data_config, num_workers=8):
    ...
    dataloader = GanDataloader(train_path, resize_type, normalize_type, mean, std,
...
def get_gan_val_dataloader(val_path, data_config, num_workers=8):
    resize_type = data_config.resize_type
    normalize_type = data_config.normalize_type
    mean = data_config.data_mean
    std = data_config.data_std
    ...
    data_channel = data_config.data_channel
    batch_size = data_config.train_batch_size
    dataloader = GanDataloader(val_path, resize_type, normalize_type, mean, std,
                               image_size, data_channel)
    result = data.DataLoader(dataset=dataloader, num_workers=num_workers,
