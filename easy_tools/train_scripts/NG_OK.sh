...
function run_onnx_convert() {
    ...
    imageDir="./.easy_log/binary_cls_img"
    outDir="${root_path}/.easy_log/out"
    modelName=binarynet
    outNetName=binarynet

    inputColorFormat=1
    outputShape=1,3,224,224
    outputLayerName="o:ng_output|ot:0,1,2,3|odf:fp32"
...
function main() {
    ...
    CUDA_VISIBLE_DEVICES=0 python3 -m easy_tools.easy_ai --task NG_OK --gpu 0 --trainPath ${dataset_train_path} --valPath ${dataset_val_path}
    if [ $? -ne 0 ]; then
