...
from easyai.loss.utility.box2d_process import bboxes_iou
...
class YoloV5Loss(BaseYoloLoss):
    ...
    def __init__(self, class_number, anchor_sizes, anchor_mask, reduction,
        ...
        super().__init__(LossName.YoloV5Loss, class_number, anchor_sizes, anchor_mask)
        ...
        det = model.module.model[-1] if False else model.model[-1]  # Detect() module
    ...
    def forward(self, outputs, batch_data=None):  # predictions, targets, model
        device = outputs.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        tcls, tbox, indices, anchors = self.build_targets(outputs, batch_data)  # targets

        # Losses
        for i, pi in enumerate(outputs):  # layer index, layer predictions
            ...
            if n:
                ...
                iou = bboxes_iou(pbox.T, tbox[i], xyxy=False, CIoU=True)  # iou(prediction, target)
