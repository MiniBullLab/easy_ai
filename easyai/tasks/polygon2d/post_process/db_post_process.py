...
class DBPostProcess(BasePostProcess):
    ...
    def __call__(self, predict_score, src_size):
        ...
        for m_contour in contours:
            if len(m_contour) < 4 and cv2.contourArea(m_contour) < 16:
                continue
            m_rotated_box = self.get_min_area_bbox(refined_mask_region, m_contour,
                                                   self.bbox_scale_ratio)
            ...
            if min(m_box_width * w, m_box_height * h) < self.shortest_length:
                continue
            rotated_points = self.get_coordinates_of_rotated_box(m_rotated_box,
                                                                 height, width)
            ...
            socre = float(np.sum(m_region_mask) / m_mask_count)

            polygon_object = Polygon2dObject()
            polygon_object.clear_polygon()
            for temp_value in rotated_points:
    ...
    def get_coordinates_of_rotated_box(self, _rotated_box, _height, _width):
        """
        获取旋转的矩形的对应的四个顶点坐标
        Args:
            _image:     对应的图像
        ...
        rotated_points = self.rotate_points(raw_points, _rotated_box['degree'],
                                            (center_x * _width, center_y * _height))
