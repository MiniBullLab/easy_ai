...
from easyai.tasks.polygon2d.polygon2d_test import Polygon2dTest
from easyai.name_manager.task_name import TaskName
from easyai.tasks.utility.task_registry import REGISTERED_TRAIN_TASK
from easyai.utility.logger import EasyLogger


@REGISTERED_TRAIN_TASK.register_module(TaskName.Polygon2d_Task)
class Polygon2dTrain(CommonTrain):
    ...
    def __init__(self, model_name, gpu_id, config_path=None):
        ...
        self.polygon2d_test = Polygon2dTest(model_name, gpu_id, self.train_task_config)
    ...
    def test(self, val_path, epoch, save_model_path):
        if val_path is not None and os.path.exists(val_path):
            if self.test_first:
                self.polygon2d_test.create_dataloader(val_path)
                self.test_first = False
            if not self.polygon2d_test.start_test():
