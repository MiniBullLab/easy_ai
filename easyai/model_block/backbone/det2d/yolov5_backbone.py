...
__all__ = ['Yolov5s_Backbone', 'Yolov5m_Backbone',
           'Yolov5l_Backbone', 'Yolov5x_Backbone', 'Yolov5OldBackBone']


class Yolov5BackBone(BaseBackbone):
    def __init__(self, data_channel=3, num_blocks=(1, 2, 3, 1),
...
@REGISTERED_CLS_BACKBONE.register_module(BackboneName.Yolov5s_Old_Backbone)
class Yolov5OldBackBone(BaseBackbone):
    def __init__(self, data_channel=3, num_blocks=(1, 3, 3, 1),
                 out_channels=(64, 128, 256, 512), strides=(2, 2, 2, 2),
                 dilations=(1, 1, 1, 1), bnName=NormalizationType.BatchNormalize2d,
                 activationName=ActivationType.Swish):
        super().__init__(data_channel)
        self.set_name(BackboneName.Yolov5s_Old_Backbone)
        self.num_blocks = num_blocks
        self.out_channels = out_channels
        self.strides = strides
        self.dilations = dilations
