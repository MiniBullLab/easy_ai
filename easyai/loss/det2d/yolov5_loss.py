...
class YoloV5Loss(BaseYoloLoss):
    def __init__(self, class_number, anchor_sizes, anchor_mask, reduction,
        ...
        self.box_loss_gain = 1.0
        self.cls_loss_gain = 1.0
        self.obj_loss_gain = 1.0
        self.iou_gain = 1.0
        self.iou_threshold = iou_threshold
        self.balance = [4.0, 1.0, 0.4] # {3: [4.0, 1.0, 0.4]}.get(det.nl, [4.0, 1.0, 0.25, 0.06, .02])  # P3-P7

        self.anchor_sizes = self.anchor_sizes / float(self.reduction)
        # loss defines
        self.cls_bce_loss = nn.BCEWithLogitsLoss(pos_weight=self.cls_bceloss_pos_weight)
        self.obj_bce_loss = nn.BCEWithLogitsLoss(pos_weight=self.obj_bceloss_pos_weight)
    ...
    def build_targets(self, pred_boxes, gt_targets, height, width, device):
        ...
        tcls = torch.zeros(batch_size, self.anchor_count, nPixels,
                           requires_grad=False, device=device)
        gridspace_gain = torch.ones(6, device=device)
        ...
        offsets = torch.tensor([[0, 0],
                            ...
                            ], device=device).float() * bias  # offsets

        anchors = self.anchor_sizes
        gain[2:6] = torch.tensor(pred_boxes.shape)[[3, 2, 3, 2]]  # xyxy gain
...
class YoloV5Loss(BaseYoloLoss):
    ...
    def __init__(self, class_number, anchor_siz阿es, anchor_mask, reduction,
                 coord_weight=3.0, noobject_weight=1.0,
                 object_weight=1.0, class_weight=1.0, iou_threshold=0.5):
        super().__init__(LossName.YoloV5Loss, class_number, anchor_sizes, anchor_mask)
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            if nt:
                ...
                j = torch.max(r, 1. / r).max(2)[0] < self.hyp['anchor_t']  # compare
                # j = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n)=wh_iou(anchors(3,2), gwh(n,2))
                t = t[j]  # filter the useful box id
