...
class Det3dPCDatasetSampling():
    ...
    def __init__(self, data_root, info_path, point_features,
                 sample_groups, classes, rate=1.0):
        super().__init__()
        ...
        for k, v in db_infos.items():
            EasyLogger.info(f'load {len(v)} {k} database infos')
        ...
        self.points_loader = PointCloudProcess(dim=point_features)

    def sample_all(self, gt_bboxes, gt_labels):
        ...
        if len(sampled) > 0:
            ...
            count = 0
            for info in sampled:
                file_path = os.path.join(self.data_root, info['path'])
                s_points = self.points_loader.read_pointcloud(file_path)
    ...
    def sample_class_v2(self, name, num, gt_bboxes):
        ...
        num_sampled = len(sampled)
        gt_bboxes_bv = box3d_op.center_to_corner_box2d(
        ...
        sp_boxes_bv = box3d_op.center_to_corner_box2d(
            sp_boxes_new[:, 0:2], sp_boxes_new[:, 3:5], sp_boxes_new[:, 6])

        total_bv = np.concatenate([gt_bboxes_bv, sp_boxes_bv], axis=0)
        coll_mat = box3d_op.box_collision_test(total_bv, total_bv)
