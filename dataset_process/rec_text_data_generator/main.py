...
def parse_arguments():
    ...
    parser.add_argument(
        "--output_dir", type=str, nargs="?", help="The output directory", default="Synthetic_text/JPEGImages"
    )
    parser.add_argument(
        "-i",
        "--input_file",
...
def container_char_generate(generate_num, file_path):
...
def main():
    ...
    # config file
    colors = ['white', 'black']
    args.output_dir = "./Synthetic_text/JPEGImages"
    args.count = int(args.count / 4)
    args.background = 3
    args.image_dir = "./files/images"
    args.font_dir = "./files/fonts"
    args.dict = ".container.txt"
    args.format = 128
    # args.random_blur = True
    # args.blur = 3
    container_char_generate(10000, args.dict)
        ...
        for color in colors:
            ...
            if args.orientation == 1:
                for i in range(string_count):
                    ...
                    im = Image.open(os.path.join(args.output_dir, file_name))

                    out = im.transpose(Image.ROTATE_90)
                    out.save(os.path.join(args.output_dir, file_name))

    # generate train val txt file
    if not os.path.exists(args.output_dir.replace("JPEGImages", "ImageSets")):
        os.makedirs(args.output_dir.replace("JPEGImages", "ImageSets"))
    file_train = open(os.path.join(args.output_dir.replace("JPEGImages", "ImageSets"), "train.txt"), "w")
    file_val = open(os.path.join(args.output_dir.replace("JPEGImages", "ImageSets"), "val.txt"), "w")
